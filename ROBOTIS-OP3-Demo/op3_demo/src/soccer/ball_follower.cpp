...
void BallFollower::setNode(rclcpp::Node::SharedPtr node)
{
  ...
  if (node_ != nullptr)
  {
    ...
    yaw_sub_ = node_->create_subscription<std_msgs::msg::Float32>("/yaw_orientation", 10,std::bind(&BallFollower::yawCallback, this, std::placeholders::_1));
    camera_pub_ = node_->create_publisher<my_msg::msg::Camera>("/ball", 10);
  }
  else
  {
...
//  int ball_position_sum = 0;
  ...
  cam_msg.distance = distance_to_ball;
