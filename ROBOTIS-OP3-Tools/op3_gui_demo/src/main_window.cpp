...
void MainWindow::updateWalkingParams(op3_walking_module_msgs::msg::WalkingParam params)
{
  ...
  ui_->dSpinBox_p_gain->setValue(params.balance_p_gain);
  ui_->dSpinBox_i_gain->setValue(params.balance_i_gain);
  ui_->dSpinBox_d_gain->setValue(params.balance_d_gain);
...
void MainWindow::applyWalkingParams()
{
  ...
  walking_param.balance_p_gain = ui_->dSpinBox_p_gain->value();
  walking_param.balance_i_gain = ui_->dSpinBox_i_gain->value();
  walking_param.balance_d_gain = ui_->dSpinBox_d_gain->value();
