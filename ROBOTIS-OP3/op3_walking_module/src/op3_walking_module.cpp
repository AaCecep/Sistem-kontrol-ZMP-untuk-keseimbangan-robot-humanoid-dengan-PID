...
void WalkingModule::queueThread()
{
  ...
  auto walking_param_sub = this->create_subscription<op3_walking_module_msgs::msg::WalkingParam>(
    ...
    /* === Tambahkan subscriber untuk topik zmp === */
  auto zmp_sub = this->create_subscription<my_msg::msg::Data>(
    "zmp", 10,
    std::bind(&WalkingModule::zmpCallback, this, std::placeholders::_1));
...
void WalkingModule::zmpCallback(const my_msg::msg::Data::SharedPtr msg)
{
  ...
  walking_param_.init_x_offset = offset_correction - 0.018;
...
void WalkingModule::loadWalkingParam(const std::string &path)
{
  ...
  // Custom code: balance gain
  walking_param_.balance_p_gain = doc["balance_p_gain"].as<double>();
  walking_param_.balance_i_gain = doc["balance_i_gain"].as<double>();
  walking_param_.balance_d_gain = doc["balance_d_gain"].as<double>();
}
...
void WalkingModule::saveWalkingParam(std::string &path)
{
  ...
  // Custom code: balance gain
  out_emitter << YAML::Key << "balance_p_gain" << YAML::Value << walking_param_.balance_p_gain;
  out_emitter << YAML::Key << "balance_i_gain" << YAML::Value << walking_param_.balance_i_gain;
  out_emitter << YAML::Key << "balance_d_gain" << YAML::Value << walking_param_.balance_d_gain;
